...
def inference(image: np.ndarray,
	...
	for i in indices:
	    ...
	    cv2.putText(image, f'{class_name} {score:.2f}', (int(box[0]), int(box[1] - 10)), 				cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
def preprocess_video(input_video_path: str, output_video_path: str, scale_percent: int = 75) -> (List[str], List[float]):
     ...
     out = cv2.VideoWriter(output_video_path, fourcc, fps // 2, (target_width,
target_height), True)
     
     # Process each frame
     total_names = [] # Result of all detection categroies in preprocessed
video
     total_scores = [] # The result scores of all detection types in
preprocessed video
     while True:
	ret, frame = cap.read()
	if not ret:
	...
	# Invoke the tensorflow lite model to detect the targe
	frame, names, scores = inference(frame, interpreter, threshold_prob)
...
def send_email(file_to_send: str):
    ...
    mimeBase.add_header('Content-Disposition', f"attachment; filename=
